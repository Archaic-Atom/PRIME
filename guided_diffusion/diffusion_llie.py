...
class mix_x0that(nn.Module):
    ...
    def forward(self, Ly, Lt):
        return Ly + self.level * Lt
...
class L_denoise(nn.Module):
    def __init__(self, alpha=1e-3, use_tv=True):
        super(L_denoise, self).__init__()
        self.alpha = alpha
        self.use_tv = use_tv
    ...
    def forward(self, x):
        ...
        fft = torch.fft.fft2(x, norm="ortho")   # shape: B x C x H x W
        fft_shift = torch.fft.fftshift(fft)
        magnitude = torch.abs(fft_shift)

        # Penalize high frequency (edges and noise)
        ...
        high_freq_loss = magnitude[..., high_freq_mask].mean()
        loss += self.alpha * high_freq_loss

        # --- Optional TV loss ---
        if self.use_tv:
            loss += self.alpha * self._tv_loss(x)
...
class Diffusion(object):
    ...
    def PRIME(self, model,cls_fn):
        ...
        for (input_y, classes),name in pbar:
            ...
            Decom = DecomNet().to(self.device)

            with torch.enable_grad():
                Decom.train()
                optimizer_decom = torch.optim.Adam(Decom.parameters(), lr=1e-3)
                l2_loss = nn.MSELoss().to(self.device)
                for _ in range(300):
                    Ry, Ly = Decom(input_y)
                    recon = Ry * Ly
                    optimizer_decom.zero_grad()
                    loss = l2_loss(recon, input_y)
                    loss.backward()
            ...
            with torch.no_grad():
                ...
                for i, j in tqdm.tqdm(time_pairs):
                    ...
                    if j < i: # normal sampling 
                        ...
                        mix_model.train()
                        certerion = L_bri()
                        denoise_loss = L_denoise(alpha=1e-3)
                        optimizer_mix = torch.optim.Adam(mix_model.parameters(), lr=1e-2)

                        if i<50:
                            with torch.enable_grad():
                                for epoch in range(50):
                                    y_plus_x0_t_L = mix_model(Ly.detach(), Lt.detach())
                                    x0_t_hat = y_plus_x0_t_L * Ry.detach()
                                    optimizer_mix.zero_grad()
                                    loss = certerion(x0_t_hat) + denoise_loss(x0_t_hat)
                                    loss.backward()
                                    optimizer_mix.step()
                        else:
                            y_plus_x0_t_L = Ly + Lt
                            x0_t_hat = y_plus_x0_t_L * Ry
